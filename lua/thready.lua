...
--- Made by fatboychummy https://github.com/Fatboychummy-CC/Libraries/tree/main and released into the public domain (thank you!)

--- Useful post: https://www.reddit.com/r/ComputerCraft/comments/1bgzb4u/are_there_any_kind_of_interupts_or_multiple/
    ...
    for _, set in pairs(thready.listeners) do
      for _, listener in ipairs(set) do
        if listener.event == event_data[1] then
          thready.spawn(listener.set_name, listener.callback, thready.websocket, table.unpack(event_data, 1, event_data.n))
        end
      end
    end
