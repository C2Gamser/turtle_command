...
impl MCDataCrawler {
    ...
    pub fn extract_texture_data(&self) {
            ...
            .run(|_, path: PathBuf| {
                if !path.to_string_lossy().contains("processedMods") {
                    ...
                    for entry in zip_contents.iter() {
                        ...
                        regex!(r"/(item)/").is_match(&entry.name) && // contains any of these surrounded by /
                        regex!(r"\.(png)$").is_match(&entry.name) // Ends with .json or .png
                            {
                            let trimmed_destination = regex!(r"(assets/|textures/|item/)").replace_all(&entry.name, "");
